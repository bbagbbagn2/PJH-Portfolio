import React from 'react';

import Layout from './layout';
import CustomSlider from './Slider';
import PostContainer from './Paragraph';

import screen1 from "./work-images/drinkable/screen1.png";
import screen2 from "./work-images/drinkable/screen2.png";
import screen3 from "./work-images/drinkable/screen3.png";

export default function Drinkable() {
    const settings = {
        dots: true,
        infinite: true,
        speed: 500,
        slidesToShow: 1,
        slidesToScroll: 1,
        autoPlay: true,
        autoplaySpeed: 2000,
    };

    const images = [
        { src: screen1, alt: "Drinkable: Screen 1." },
        { src: screen2, alt: "Drinkable: Screen 2." },
        { src: screen3, alt: "Drinkable: Screen 3." },
    ];

    const paragraphs = [
        "DRINKABLE은 칵테일의 종류에 대해 알려주는 웹사이트입니다.",

        "TypeScript 및 백엔드를 공부하기 위해 개발한 개인 프로젝트입니다.",

        "전체적인 웹사이트의 UI를 디자인하였습니다. 또한, 로그인 및 회원가입, 위시리스트 등 axios를 통하여 백엔드와의 연결을 구축하였습니다." + 
        "MySQL과 데이터베이스를 만들었고, Node.js 및 express를 통해 백엔드를 구축하였습니다." + 
        "백엔드를 처음 사용하기 때문에 긴 시간이 걸렸지만, 완성된 프로젝트의 백엔드 코드를 자세히 찾아보고 공부하여 백엔드 코드를 완성할 수 있었습니다.",

        "해당 프로젝트는 HTML, CSS, TypeScript, React, style-components, Node.js, express, MySQL과 같은 기술을 사용하였습니다." +
        "저는 TypeScript 및 백엔드 공부를 함으로써 프론트엔드에 대한 지식을 습득함과 동시에 백엔드와의 협업 시, 더욱 원할한 의사소통을 할 수 있는 경험을 얻었습니다.",

        "TypeScript 및 데이터베이스를 활용한 개인 프로젝트였기 때문에 해당 기술에 대해 많은 공부를 하였고 많은 피드백을 얻었습니다.",
    ];

    return (
        <Layout>
            <CustomSlider settings={settings} images={images} />
            <PostContainer paragraph={paragraphs} />
        </Layout>
    );
}